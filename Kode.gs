...
function doPost(e) {
  try {
    ...
    const { uid, email, nama } = data;

    // Cek apakah email sudah ada
    const emails = sheet.getRange(2, 1, sheet.getLastRow() - 1, 1).getValues().flat();
    if (emails.includes(email)) {
      return ContentService.createTextOutput("User sudah login sebelumnya.");
    }

    sheet.appendRow([email, uid, nama]);
    return ContentService.createTextOutput("Login berhasil!");
  } catch (err) {
    return ContentService.createTextOutput("Gagal login: " + err.message);
  }
}
